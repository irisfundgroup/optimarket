  ...
  if (confirmed) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-2xl font-black text-white mb-2">Investissement activé !</h2>
        <p className="text-slate-400 mb-2">Vous avez investi <span className="text-white font-bold">{amountNum.toLocaleString()} XOF</span></p>
        <p className="text-slate-400 mb-6">Gain estimé dans {durationDays} jours: <span className="text-emerald-400 font-bold">+{gains.toFixed(0)} XOF</span></p>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 md:px-8 py-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-lg font-bold text-white">Activer l'opportunité</h1>
      </div>
      ...
      {opportunity && (
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}>
          ...
          <div className="flex items-center gap-4 text-sm">
            <span className="text-emerald-400 font-bold">+{returnPercent}% de retour</span>
            <span className="text-slate-400">en {durationDays} jours</span>
      ...
      <div className="space-y-4 mb-6">
        <div>
          <Label className="text-white text-sm mb-2 block">Montant à investir (XOF)</Label>
          <Input
            type="number"
          ...
          />
          <p className="text-xs text-slate-500 mt-1">Niveau {level.toUpperCase()}: max {MAX_INVEST.toLocaleString()} XOF · Frais: {lvlConfig.fees}%</p>
        </div>
        ...
        {amountNum >= MIN_INVEST && (
          <div className="rounded-2xl p-4 space-y-2" style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.2)' }}>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Frais ({lvlConfig.fees}%)</span>
              <span className="text-red-400">-{fees.toFixed(0)} XOF</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Gain estimé (+{returnPercent}%)</span>
              <span className="text-emerald-400 font-bold">+{gains.toFixed(0)} XOF</span>
            </div>
            <div className="border-t border-white/10 pt-2 flex justify-between">
              <span className="text-slate-300 font-semibold">Total après {durationDays}j</span>
      ...
      >
        {activateMutation.isPending ? (
          ...
          <><Calculator className="w-5 h-5" /> Confirmer l'investissement</>
        )}
      </Button>
    </div>
  );
